#!/usr/bin/env python3
"""Popcorn/Brev driver for the B200 (16, 512, 512) specialization."""
...
VARIANT_NAMES = (
    ...
    "staged64_fp32_raw_scalar_u256_cutlass_names",
)
...
def _source_path() -> Path:
    return Path(__file__).with_name("cholesky_b16n512.py").resolve()
...
def _autotune(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        path = submission_dir / (
            f"cholesky_b16n512_v{variant:02d}.py"
        )
...
def _ncu(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        ...
        variant_dir.mkdir()
        submission = (
            variant_dir / f"cholesky_b16n512_v{variant:02d}.py"
        ).resolve()
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Autotune, profile, or submit the B200 (16,512,512) "
