#!/usr/bin/env python3
"""Popcorn/Brev driver for the B200 (4, 1024, 1024) specialization."""
...
def _source_path() -> Path:
    return Path(__file__).with_name("cholesky_b4n1024.py").resolve()
...
def _autotune(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        path = submission_dir / (
            f"cholesky_b4n1024_v{variant:02d}.py"
        )
...
def _ncu(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        ...
        variant_dir.mkdir()
        submission = (
            variant_dir / f"cholesky_b4n1024_v{variant:02d}.py"
        ).resolve()
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(4,1024,1024) Cholesky specialization"
        )
    )
    parser.add_argument(
        "--popcorn",
