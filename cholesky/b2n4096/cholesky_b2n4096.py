...
at::Tensor cholesky_b2n4096(const at::Tensor& data, int64_t variant);
void cholesky_b2n4096_out(
    const at::Tensor& data, at::Tensor out, int64_t variant);
at::Tensor cholesky_b2n4096_metadata();
void cholesky_b2n4096_profile(bool enabled);
void cholesky_b2n4096_hybrid_copy(
    const at::Tensor& data, at::Tensor out);
void cholesky_b2n4096_hybrid_stage(
    at::Tensor out, at::Tensor host_panel,
    int64_t begin, int64_t width);
void cholesky_b2n4096_hybrid_finish(
    at::Tensor out, const at::Tensor& host_factor,
    int64_t begin, int64_t width);

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("prepare", &cholesky_b2n4096_prepare,
        "Configure one B200 2x4096 Cholesky variant");
  m.def("run", &cholesky_b2n4096, "Batched 2x4096 Cholesky");
  m.def("run_out", &cholesky_b2n4096_out,
        "Batched 2x4096 Cholesky out");
  m.def("metadata", &cholesky_b2n4096_metadata,
        "B200 kernel resource metadata");
  m.def("profile", &cholesky_b2n4096_profile,
        "Toggle hybrid NVTX ranges");
  m.def("hybrid_copy", &cholesky_b2n4096_hybrid_copy,
        "Initialize the hybrid output");
  m.def("hybrid_stage", &cholesky_b2n4096_hybrid_stage,
        "Run a hybrid diagonal and history phase");
  m.def("hybrid_finish", &cholesky_b2n4096_hybrid_finish,
...
void gemm_inner(
  ...
  check_cublas(
      cublasGemmStridedBatchedEx(
          ...
          &alpha,
          micro_rows, CUDA_R_32F, kN, kMatrixStride,
          micro_rows, CUDA_R_32F, kN, kMatrixStride,
          &beta,
          destination, CUDA_R_32F, kN, kMatrixStride,
          kBatch,
...
void gemm_apply_column(
  ...
  check_cublas(
      cublasGemmStridedBatchedEx(
          ...
          static_cast<int64_t>(Micro) * Micro,
          x_rows, CUDA_R_32F, kN, kMatrixStride,
          &beta,
...
void launch_wedges(float* output, int nb) {
  ...
  for (int batch = 0; batch < kBatch; ++batch) {
    cudaLaunchKernelEx(
        &config, zero_wedges_kernel,
        output + static_cast<int64_t>(batch) * kMatrixStride, nb);
  }
}
      ...
      if constexpr (V::fused) {
        for (int batch = 0; batch < kBatch; ++batch) {
          launch_fused_micro<
              ...
              t_inv + static_cast<int64_t>(batch) * Micro * Micro,
              flags + batch * (kN / V::min_micro));
        }
      } else {
        for (int batch = 0; batch < kBatch; ++batch) {
          launch_factor<Micro>(
        ...
        if (micro + Micro < kN) {
          ...
          for (int batch = 0; batch < kBatch; ++batch) {
            launch_copy_back<Micro>(
                output +
                    static_cast<int64_t>(batch) * kMatrixStride,
                scratch + static_cast<int64_t>(batch) *
                    (kN - Micro) * Micro,
                static_cast<int>(micro));
          }
        }
    ...
    if (width == 128) {
      for (int batch = 0; batch < kBatch; ++batch) {
        launch_fused_micro<128, false, 1>(
            output + static_cast<int64_t>(batch) * kMatrixStride,
            begin,
            t_inv + static_cast<int64_t>(batch) * 128 * 128,
            flags + batch * (kN / 32));
      }
    } else if (width == 64) {
      for (int batch = 0; batch < kBatch; ++batch) {
...
void ensure_hybrid_state() {
  ...
  if (!gHostPanel64.defined()) {
    gHostPanel64 = at::empty({kBatch, 64, 64}, host_float);
    gHostFactor64 = at::empty({kBatch, 64, 64}, host_float);
    gHostInfo64 = at::empty({kBatch}, host_int);
  }
  if (!gHostPanel128.defined()) {
    gHostPanel128 = at::empty({kBatch, 128, 128}, host_float);
    gHostFactor128 = at::empty({kBatch, 128, 128}, host_float);
    gHostInfo128 = at::empty({kBatch}, host_int);
  }
}
...
void check_host_panel(const at::Tensor& panel, int width) {
  ...
  TORCH_CHECK(
      panel.dim() == 3 && panel.size(0) == kBatch &&
  ...
  }
  {
    ...
    for (int batch = 0; batch < kBatch; ++batch) {
      check_cuda(
          cudaMemcpy2DAsync(
              host_panel +
                  static_cast<int64_t>(batch) * width * width,
              static_cast<size_t>(width) * sizeof(float),
              output +
                  static_cast<int64_t>(batch) * kMatrixStride +
                  static_cast<int64_t>(begin) * kN + begin,
    ...
    for (int batch = 0; batch < kBatch; ++batch) {
      ...
      check_cublas(
          cublasStrsm(
              ...
              width, rows, &alpha,
              matrix + static_cast<int64_t>(begin) * kN + begin,
              kN,
              matrix + static_cast<int64_t>(begin + width) * kN +
                  begin,
              kN),
...
void launch_hybrid(
  ...
  for (int begin = 0; begin < kN; begin += width) {
    ...
    for (int batch = 0; batch < kBatch; ++batch) {
      TORCH_CHECK(
          host_info.data_ptr<int>()[batch] == 0,
...
void check_input(const at::Tensor& data) {
  ...
  TORCH_CHECK(
      ...
      "native input must have shape (2, 4096, 4096)");
}
...
void cholesky_b2n4096_out(
  ...
  at::Tensor t_inv = at::empty(
      {static_cast<int64_t>(kBatch) * micro * micro},
      data.options());
  at::Tensor flags;
  ...
  if (kVariantUsesScratch[selected]) {
    scratch = at::empty(
        {static_cast<int64_t>(kBatch) * (kN - micro) * micro},
        data.options());
...
def _native_module():
    ...
    try:
        return load_inline(
            name=f"cholesky_b2n4096_b200_{tag}",
...
def _compiled_hybrid_buffers() -> tuple[torch.Tensor, torch.Tensor]:
    panel = torch.empty(
        (2, 64, 64), dtype=torch.float32, pin_memory=True
    )
...
def _run_variant(
    ...
    if variant == 0:
        ...
        return factor
    if variant == 11:
        # cuSOLVER's batched POTRF is built for many small matrices and is
        # pathological at n=4096: entry [11] costs 11.177 ms while the
        # single-matrix path factors the same n in 1.528 ms at batch 1.
        # Factor each matrix separately so every call takes that path.
        factor = out if out is not None else torch.empty_like(data)
        for index in range(data.shape[0]):
            factor[index] = torch.linalg.cholesky_ex(
