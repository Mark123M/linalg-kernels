#!/usr/bin/env python3
"""Popcorn/Brev driver for the B200 (2, 4096, 4096) specialization."""
...
def _source_path() -> Path:
    return Path(__file__).with_name("cholesky_b2n4096.py").resolve()
...
def _autotune(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        path = submission_dir / (
            f"cholesky_b2n4096_v{variant:02d}.py"
        )
...
def _ncu(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        ...
        variant_dir.mkdir()
        submission = (
            variant_dir / f"cholesky_b2n4096_v{variant:02d}.py"
        ).resolve()
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(2,4096,4096) Cholesky specialization"
        )
    )
    parser.add_argument(
        "--popcorn",
