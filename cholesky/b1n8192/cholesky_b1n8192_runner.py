#!/usr/bin/env python3
"""Popcorn/Brev driver for the B200 (1, 8192, 8192) specialization."""
...
def _source_path() -> Path:
    return Path(__file__).with_name("cholesky_b1n8192.py").resolve()
...
def _preflight_output(popcorn: str) -> str:
    ...
    if completed.returncode != 0 or "--output" not in help_text:
        raise RuntimeError(
            "the selected Popcorn CLI does not expose --output; no "
            "source was promoted"
        )
    return help_text
...
def _summary_json(text: str) -> dict[str, Any]:
    ...
    for index in range(len(lines) - 1, -1, -1):
        ...
        if isinstance(payload, dict):
            return payload
    raise ValueError("Popcorn output has no submission summary object")
...
def _extract_timings(payload: Any) -> dict[str, decimal.Decimal]:
    ...
    if not results:
        raise ValueError("JSON contains no public benchmark result")
    ...
    for result_index, result in enumerate(results):
        ...
        if result.get("check") != "pass":
            raise ValueError(
                f"public benchmark check is {result.get('check')!r}"
            )
    ...
    if len(target_rows) != 1:
        raise ValueError(
            "expected exactly one public benchmark result, found "
            f"{len(target_rows)}"
        )
...
def _autotune(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        path = submission_dir / (
            f"cholesky_b1n8192_v{variant:02d}.py"
        )
    ...
    if not ranking:
        ...
        raise RuntimeError(
            "no variant passed every round; inspect "
            f"{summary_path}"
        )
    ...
    beat_baseline = winner_mean < BASELINE_TARGET_NS
    if beat_baseline:
        promotion = _atomic_promote(
            source_path, source, source_hash, winner
        )
        promoted_variant: int | None = winner
    else:
        promotion = "retained_default_no_baseline_win"
    ...
    _write_json(
        summary_path,
        {
            ...
            "baseline_target_ns": str(BASELINE_TARGET_NS),
    ...
    print(
        ...
        f"{ranking[0]['median_mean_ns']} "
        f"baseline_ns={BASELINE_TARGET_NS} promotion={promotion}",
        flush=True,
    )
    return summary_path
...
def _ncu(args: argparse.Namespace) -> Path:
    ...
    for variant in variants:
        ...
        variant_dir.mkdir()
        submission = (
            variant_dir / f"cholesky_b1n8192_v{variant:02d}.py"
        ).resolve()
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(1,8192,8192) Cholesky specialization"
        )
    )
    parser.add_argument(
        "--popcorn",
